...
metadata <- metadata[,-1]
#colnames(metadata)[2] = ""
levels(metadata$SeedSanitization)
metadata$SeedSanitization.ord = factor(metadata$SeedSanitization, c("Sanitized", "NotSanitized"))
levels(metadata$SeedSanitization.ord)
metadata$EcoliTreatment.ord = factor(metadata$EcoliTreatment, c("Treated", "NotTreated"))
levels(metadata$EcoliTreatment.ord)
...
diagdds = phyloseq_to_deseq2(physeq_picrust, ~ SeedSanitization)
diagdds = DESeq(diagdds, test="Wald", fitType="parametric")
diagddsE = phyloseq_to_deseq2(physeq_picrust, ~ EcoliTreatment)
diagddsE = DESeq(diagddsE, test="Wald", fitType="parametric")
#the test type of "Wald" tests for significance of coefficients in a Negative Binomial GLM. 
#This is generally a pretty good assumption for sequencing experiments. 
#This was designed with RNA-seq in mind, but also pretty good for 16S sequencing.
...
alpha = 0.05
my_contrast = c("SeedSanitization", "Sanitized", "NotSanitized") 
my_contrastE = c("EcoliTreatment", "Treated", "NotTreated") 

res = results(diagdds, contrast = my_contrast, cooksCutoff = FALSE)
resE = results(diagddsE, contrast = my_contrastE, cooksCutoff = FALSE)

sigtab = res[which(res$padj < alpha), ]
sigtab <- as(sigtab, "data.frame")
sigtab2 <- subset(sigtab, abs(log2FoldChange) > 2)
#sigtab = cbind(as(sigtab, "data.frame"), as(tax_table(physeq_deseq)[rownames(sigtab), ], "matrix"))
head(sigtab)

sigtabE = resE[which(resE$padj < alpha), ]
sigtabE <- as(sigtabE, "data.frame")
sigtabE2 <- subset(sigtabE, abs(log2FoldChange) > 2)
#sigtab = cbind(as(sigtab, "data.frame"), as(tax_table(physeq_deseq)[rownames(sigtab), ], "matrix"))
head(sigtabE)
...
str(sigtab)
sigtabE$pathway <- row.names(sigtabE)
str(sigtabE)
pathway_mapfile <- read.delim("metacyc_pathways_info.txt", sep = "\t", header = F, quote = "", stringsAsFactors = F)
str(pathway_mapfile)
...
sigtabE <- inner_join(sigtabE, pathway_mapfile, by = c("pathway" = "abbreviation"))
...
xE = tapply(sigtabE$log2FoldChange, sigtabE$pathway_name, function(xE) max(xE))
xE = sort(xE, TRUE)
sigtabE$pathway_name = factor(as.character(sigtabE$pathway_name), levels=names(xE))
...
DESeq_figE = ggplot(sigtabE, aes(x = pathway_name, y = log2FoldChange)) + 
  geom_point(size=3) + 
  ylab(paste0("(", my_contrastE[2], "/", my_contrastE[3], ")\n", "log2FoldChange")) +
  #scale_color_manual(values = my_colors[c(4,6,8,10,12,14,16,18,20)]) +
  #ylim(0,8) +
  theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.2))+
  theme(axis.text.x = element_text(color = "black", size = 10),
...
ggsave(paste0("R_output/DESeq2-", my_contrast[2], "-", my_contrast[3], ".png"), DESeq_fig, height = 10, width = 40)
ggsave(paste0("R_output/DESeq2-", my_contrastE[2], "-", my_contrastE[3], ".png"), DESeq_figE, height = 10, width = 40)
...
sigtab = res[which(res$padj < alpha), ]
sigtab <- as(sigtab, "data.frame")
sigtab <- subset(sigtab, abs(log2FoldChange) > 2)
sigtabE = resE[which(resE$padj < alpha), ]
sigtabE <- as(sigtabE, "data.frame")
sigtabE <- subset(sigtabE, abs(log2FoldChange) > 2)
#Let's look at the OTUs that were significantly different between the two treatment groups. The following makes a nice ggplot2 summary of the results.
...
DESeq_figE = ggplot(sigtabE, aes(x = pathway_name, y = log2FoldChange)) + 
  ...
  #scale_color_manual(values = my_colors[c(4,6,8,10,12,14,16,18,20)]) +
  #ylim(0,8) +
  geom_text(color="black", x=length(unique(sigtabE$pathway_name))-5, y=max(sigtabE$log2FoldChange)-1, label=my_contrastE[2], show_guide = F) +
  geom_text(color="black", x=5, y=min(sigtabE$log2FoldChange)+1, label=my_contrastE[3], show_guide = F) +
  theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.2))+
  theme(axis.text.x = element_text(color = "black", size = 10),
        axis.text.y =element_text(color = "black", size = 12)) 

ggsave(paste0("R_output/DESeq2-", my_contrast[2], "-", my_contrast[3], "-filtered.png"), DESeq_fig, height = 8, width = 15)
ggsave(paste0("R_output/DESeq2-", my_contrastE[2], "-", my_contrastE[3], "-filtered.png"), DESeq_figE, height = 8, width = 15)
...
run_deseq2 <- function(my_factor, x, y){
  ...
  ggsave(paste0("R_output/DESeq2-", my_contrast[2], "-", my_contrast[3], "-filt.png"), DESeq_fig, height = 10, width = 20)
}

run_deseq2E <- function(my_factor, x, y){
  
  my_contrast <- c(my_factor, x, y)
  res = results(diagddsE, contrast = my_contrast, cooksCutoff = FALSE)
...
run_deseq2("SeedSanitization", "Sanitized", "NotSanitized")
run_deseq2E("EcoliTreatment", "Treated", "NotTreated")
