...
if(!dir.exists("R_output/taxa"))
  dir.create("R_output/taxa")
...
physeq <- qza_to_phyloseq(
  ...
  metadata = "MetaData_complete2.txt"
)
...
metadata$SeedSanitization.ord = factor(metadata$SeedSanitization, c("NotSanitized", "Sanitized"))
...
filtered_taxonomy <- taxonomy %>%
  filter(!grepl("mitochondria", Kingdom, ignore.case = TRUE))
filtered_taxonomy <- filtered_taxonomy %>%
  filter(!grepl("mitochondria", Phylum, ignore.case = TRUE))
filtered_taxonomy <- filtered_taxonomy %>%
  filter(!grepl("mitochondria", Class, ignore.case = TRUE))
filtered_taxonomy <- filtered_taxonomy %>%
  filter(!grepl("mitochondria", Order, ignore.case = TRUE))
filtered_taxonomy <- filtered_taxonomy %>%
  filter(!grepl("mitochondria", Family, ignore.case = TRUE))
filtered_taxonomy <- filtered_taxonomy %>%
  filter(!grepl("mitochondria", Genus, ignore.case = TRUE))
filtered_taxonomy <- filtered_taxonomy %>%
  filter(!grepl("mitochondria", Species, ignore.case = TRUE))

filtered_taxonomy <- filtered_taxonomy %>%
  filter(!grepl("chloroplast", Kingdom, ignore.case = TRUE))
filtered_taxonomy <- filtered_taxonomy %>%
  filter(!grepl("chloroplast", Phylum, ignore.case = TRUE))
filtered_taxonomy <- filtered_taxonomy %>%
  filter(!grepl("chloroplast", Class, ignore.case = TRUE))
filtered_taxonomy <- filtered_taxonomy %>%
  filter(!grepl("chloroplast", Order, ignore.case = TRUE))
filtered_taxonomy <- filtered_taxonomy %>%
  filter(!grepl("chloroplast", Family, ignore.case = TRUE))
filtered_taxonomy <- filtered_taxonomy %>%
  filter(!grepl("chloroplast", Genus, ignore.case = TRUE))
filtered_taxonomy <- filtered_taxonomy %>%
  filter(!grepl("chloroplast", Species, ignore.case = TRUE))

head(filtered_taxonomy)

tax.clean <- filtered_taxonomy
...
my_colors <- c(
  ...
  "#8569D5", "#5E738F","#D1A33D", "#8A7C64", "#599861", "gray", "black", "blue", "red", "purple", "yellow", "green", "orange"
)

#If you want different taxonomic level, find and replace the taxonomic level listed here
my_level <- c("Family", "Genus", "Species")
my_column <- "SeedSanitization"  #this is the metadata column that we will use in the taxa barplot
my_column_ordered <- c("Sanitized", "NotSanitized")
...
for(ml in my_level){
  ...
  ggsave(paste0("R_output/taxa/", ml, "BarPlot_", my_column, ".png"), height = 5, width = 4)
}
...
#converts your phyloseq-format microbiome data into a DESeqDataSet with dispersions estimated, using 
#the experimental design formula, also shown (the ~body.site term). The DESeq function does the rest 
#of the testing, in this case with default testing framework, but you can actually use alternatives.


diagdds = phyloseq_to_deseq2(physeq_deseq, ~ SeedSanitization)
...
run_deseq2 <- function(my_factor, x, y){
  ...
  ggsave(paste0("R_output/taxa/DESeq2-", my_contrast[2], "-", my_contrast[3], ".png"), DESeq_fig, height = 5, width = 10)
}

############# Ecoli Treatment
diagdds = phyloseq_to_deseq2(physeq_deseq, ~ EcoliTreatment)
diagdds = DESeq(diagdds, test="Wald", fitType="parametric")
alpha = 0.05
run_deseq2("EcoliTreatment", "Treated", "NotTreated")
run_deseq2 <- function(my_factor, x, y){
  my_contrast <- c(my_factor, x, y)
  res = results(diagdds, contrast = my_contrast, cooksCutoff = FALSE)
  sigtab = res[which(res$padj < alpha), ]
...
############# Roots vs Leaves
diagdds = phyloseq_to_deseq2(physeq_deseq, ~ Tissue)
diagdds = DESeq(diagdds, test="Wald", fitType="parametric")
alpha = 0.05
run_deseq2("Tissue", "PlantLeaves", "PlantRoots")
run_deseq2 <- function(my_factor, x, y){
  my_contrast <- c(my_factor, x, y)
  res = results(diagdds, contrast = my_contrast, cooksCutoff = FALSE)
  sigtab = res[which(res$padj < alpha), ]
...
#run_deseq2("Tissue", "PlantRoots", "Seeds")
#run_deseq2("Tissue", "Seeds", "PlantLeaves")
#run_deseq2("AgeAtHarvest", "0 days", "7 days")
#run_deseq2("AgeAtHarvest", "7 days", "14 days")
#run_deseq2("AgeAtHarvest", "14 days", "21 days")
#run_deseq2("AgeAtHarvest", "21 days", "28 days")
#run_deseq2("AgeAtHarvest", "0 days", "28 days")
