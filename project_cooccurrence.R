...
treatments<-as.vector(unique(dataset$SeedSanitization))
datasetn<-dataset
datasetn[datasetn==0]<-NA

#i = 1

summary(metadata$SeedSanitization)

my_column <- "SeedSanitization"
n1 <- 25
n2 <- 25
...
Sanitized_ASVs<-subset(dataset, get(my_column)==treatments[1])
NotSanitized_ASVs<-subset(dataset, get(my_column)==treatments[2])

colnames(Sanitized_ASVs[1:10])
colnames(NotSanitized_ASVs[1:10])

head(final_results)
ggplot(Sanitized_ASVs, aes(x = ASV215, y = ASV363)) +
  geom_point()
ggplot(NotSanitized_ASVs, aes(x = ASV69, y = ASV35)) +
  geom_point() ######keep this
...
write.csv(strong_results_taxa, "R_output/strong-results-taxa.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="Sanitized"), "R_output/strong-results-taxa-Sanitized.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="NotSanitized"), "R_output/strong-results-taxa-NotSanitized.csv", row.names = F)
